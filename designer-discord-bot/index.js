...
const DEFAULT_CHANNEL_ID = process.env.DESIGNER_CHANNEL || process.env.CHANNEL_ID;
const DIRECT_CHANNEL_ID = process.env.DESIGNER_CHANNEL || process.env.CHANNEL_ID;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/deliver') {
    ...
    req.on('end', () => {
      try {
        const { task, channelKey = "coder" } = JSON.parse(body);
        if (!task) {
          res.writeHead(400);
